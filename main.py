...
### FINAL ANSWER
""")
    return prompt | load_llm() | StrOutputParser()
...
def layout_chunking(text: str, page: int) -> list[dict]:
    sections = re.split(r'\n\s*\d+\.\s+', text)
    records = []
    for sec in sections:
        sec = sec.strip()
        if len(sec) > 80:
            records.append({"content": sec, "page": page})
    return records


def create_records(ocr_data: list[dict]) -> list[dict]:
    records = []
    for doc in ocr_data:
        chunks = layout_chunking(doc["text"], doc["page"])
        records.extend(chunks)
    return records
...
def build_vector_db(records: list[dict], embeddings: HuggingFaceEmbeddings) -> Chroma:
    """Build an in-memory Chroma vector store using the shared embeddings instance."""
    texts = [r["content"] for r in records]
    metadatas = [{"page": r["page"]} for r in records]
    return Chroma.from_texts(
        ...
        embedding=embeddings,
        metadatas=metadatas,
        # No persist_directory — ephemeral per-session store
    )
...
# ── FULL PDF PROCESSING (CACHED BY PATH) ──────────────────────────────────────

@st.cache_resource
def process_pdf(pdf_path: str):
    """OCR → chunk → vector DB → BM25. Cached so it only runs once per file."""
    embeddings = load_embeddings()
    ocr_data = extract_text_from_pdf(pdf_path)
    records = create_records(ocr_data)
    vector_db = build_vector_db(records, embeddings)
    bm25 = build_bm25(records)
    return vector_db, bm25, records
...
def hybrid_search(
    ...
    keyword_docs = [records[i] for i in top_indices]
...
if uploaded_file:
    # Write upload to a unique temp file to avoid multi-user collisions
    ...
    try:
        with st.spinner("Processing PDF..."):
            vector_db, bm25, records = process_pdf(pdf_path)  # uses cache after first run
        st.success("PDF processed successfully!")
    finally:
    ...
    if question:
        vector_docs, keyword_docs = hybrid_search(question, vector_db, bm25, records)

        # Deduplicate context chunks before sending to LLM
        seen: set[str] = set()
        ...
        if "Not enough information" in answer:
            ...
            st.write("Sources (pages):", sorted_pages)